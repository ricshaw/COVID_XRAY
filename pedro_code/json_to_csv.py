...
elif dataset_flag == 'Kaggle':
    # For kaggle dataset
    dataset_directory = '/data/COVID/Data/Kaggle_Covid'
    json_directory = os.path.join(dataset_directory, 'annotations')
else:
    print('No valid flag passed!')
    sys.exit(0)
...
if dataset_flag == 'covid-19':
    ...
    for json_file in pathlib.Path(json_directory).rglob('*.json'):
        with open(os.path.join(json_directory, json_file)) as jf:
            # Load data and identify annotations
            data = json.load(jf)
            # Annotations are all kept in one field: images, can loop through all entries
            images_info = data['images']
            for image_info in images_info:
                Filename.append(image_info['file_name'])
                AccessionID.append('N/A')
                Examination_Title.append(image_info['metadata']['modality'])
                Age.append(int(image_info['metadata']['modality']))
                Gender.append(sex_survival_ICU_exam_label_converter(sex[0], tag_identifier='Sex'))
                SymptomOnset_DTM.append('N/A')
                Death_DTM.append('N/A')
                Died.append()
