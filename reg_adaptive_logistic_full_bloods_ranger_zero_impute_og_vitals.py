...
class Model(nn.Module):
    def __init__(self):
        ...
        self.classifier = nn.Sequential(nn.Linear(hidden2, out_features=1, bias=True),
                                        nn.Softplus())
...
sub.to_csv(os.path.join(SAVE_PATH, f'preds-mse-{folds_mse}-mae-{folds_mae}.csv'), index=False)
