...
class ImageDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        features = np.concatenate((bloods, vitals, age, gender, ethnicity, days_from_onset_to_scan), axis=0)
...
temp_bloods = pd.concat([bloods, vitals, age, gender, ethnicity, days_from_onset_to_scan], axis=1, sort=False)
...
for fold in range(latest_fold, FOLDS):
    ...
    if LOAD and num_files > 0 and arguments.mode == 'pretrained':
        ...
        try:
            append_string = 'best'
            loaded_model_file = f'model_epoch_{loaded_epoch}_fold_{fold}_{append_string}.pth'
            checkpoint = torch.load(os.path.join(SAVE_PATH, loaded_model_file), map_location=torch.device('cuda:0'))
        except:
            append_string = 'nb'
            loaded_model_file = f'model_epoch_{loaded_epoch}_fold_{fold}_{append_string}.pth'
    ...
    if arguments.mode == 'train' or arguments.mode == 'pretrained':
        ...
        for epoch in range(latest_epoch+1, EPOCHS):
            ...
            # Save model
            if SAVE:
                MODEL_PATH = os.path.join(SAVE_PATH, f'model_epoch_{epoch}_fold_{fold}_{append_string}.pth')
                print(MODEL_PATH)
                # if epoch != (EPOCHS - 1):
